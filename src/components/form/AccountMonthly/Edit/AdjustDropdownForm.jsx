...
const AdjustDropdownForm = ({ acctNum, yearMonth }) => {
    ...
    const initialFormData = {
        ...
        use_yn: true, // boolean toggle
        tax_free_yn: true, // boolean toggle
    };
    ...
    return (
        <div className="relative inline-block float-right">
            ...
            <DropdownMenu isOpen={isOpen} closeDropdown={closeDropdown} title="조정 정보 추가">
                <div className="px-4 py-2 space-y-3">
                    ...
                    <div className="flex flex-col">
                        ...
                        <select name="adjustment_category" value={formData.adjustment_category} onChange={handleChange}
                                className="border rounded-md p-2 text-sm">
                            <option value="overpayment">과오납</option>
                            <option value="nonpayment">미납</option>
                            <option value="subscribe">가입비</option>
                            <option value="vms_use">VMS 사용료</option>
                        </select>
                    </div>
                    ...
                    <div className="flex flex-col">
                        ...
                        <select name="adjustment_type" value={formData.adjustment_type} onChange={handleChange}
                            ...
                            <option value="addition">가산</option>
                        </select>
                    </div>
                    ...
                    <div className="flex flex-col">
                        ...
                        <select name="mount_type" value={formData.mount_type} onChange={handleChange}
                                className="border rounded-md p-2 text-sm">
                            <option value="pay">요금</option>
                            <option value="percent">요율</option>
                        </select>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex flex-row space-x-2 items-center">
                            <span className="text-blue-500">
                                {formData.tax_free_yn ? '부가세 계산 전 조정' : '부가세 계산 후 조정'}
                            </span>
                            <button
                                className={`w-10 h-5 flex items-center transition duration-300 rounded-full p-1 ${formData.tax_free_yn ? 'bg-blue-500' : 'bg-gray-300'}`}
                                onClick={() => handleToggleChange('tax_free_yn')}
                            >
                                <div
                                    className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform ${formData.tax_free_yn ? 'translate-x-5' : 'translate-x-0'}`}
                                ></div>
                            </button>
                        </div>
                    </div>
                ...
                <div className="flex justify-end p-2 bg-gray-100 rounded-b-md space-x-2">
                    ...
                    <button onClick={closeDropdown}
                            className="px-3 py-1 bg-gray-500 text-white rounded-md text-sm">Close
                    </button>
                </div>
            </DropdownMenu>
        </div>
